defmodule PixelgameWeb.AuthController do
  ...
  def callback(%{assigns: %{ueberauth_auth: auth}} = conn, _params) do
    case Accounts.signup_via_github(auth.info.email, auth.info, auth.credentials.token) do
      {:ok, user} ->
        IO.inspect(user, label: "user path")
        conn |> put_flash(:info, "Success") |> put_session(:user_id, user.id) |> redirect(to: "/")
